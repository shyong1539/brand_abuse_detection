...
class DomainThread(threading.Thread):
    ...
    def answer_to_list(answers):
        return sorted(list(map(
            lambda record: str(record).strip(".") if len(str(record).split(' ')) == 1 else str(record).split(' ')[
                1].strip('.'), answers)))
                ...
                if 'dns-a' in domain:
                    try:
                        req = requests.get(
                            ...
                            timeout=REQUEST_TIMEOUT_HTTP, headers={'User-Agent': 'Mozilla/5.0 (brand_abuse_detection)'})
    ...
    if args.ssdeep and MODULE_SSDEEP and MODULE_REQUESTS:
        ...
        try:
            req = requests.get(url.get_full_uri(), timeout=REQUEST_TIMEOUT_HTTP,
                               headers={'User-Agent': 'Mozilla/5.0 (brand_abuse_detection)'})
